 ...
 * <p>
 * Using {@link #getStringInitParameter(ServletConfig, String)} this class allows one to retrieve configured init parameters by searching first for an init
 * parameter of the {@link ServletConfig}, followed by an init parameter of the entire {@link ServletContext}. In this way, the servlet context can set a
 * property that may be overridden by the servlet config for an individual servlet.
 * </p>
...
public class Servlets {
	 ...
	 * entire {@link ServletContext}. In this way, the servlet context can set a property that may be overridden by the servlet config for an individual servlet
	 * @param servletConfig The servlet configuration.
	 * @param name The name of the init parameter.
	 * @return The configured init parameter, either from the servlet config or the servlet context, or <code>null</code> if the indicated init param was not
	 *         found.
	 ...
	 * @return The configured enum init parameter, either from the servlet config or the servlet context, or <code>null</code> if the indicated init param was not
	 *         found.
	 ...
	 * @return A file representing the server's real path of the <code>/WEB-INF</code> directory, or <code>null</code> if the real path of the directory is not
	 *         available (if the application is being served from a .war file, for instance).
	 */
	...
	/**
	 * Determines the data directory. The data directory is determined in this order:
	 * <ol>
	 ...
	 * @return A file representing the preferred data directory, or <code>null</code> if the directory is not available (if a <code>WEB-INF</code>-based directory
	 *         is requested and the application is being served from a .war file, for instance).
	...
	/**
	 * Determines the correct data subdirectory. The data directory is determined in this order:
	 * <ol>
	 ...
	 * @return A file representing the preferred log directory, or <code>null</code> if the directory is not available (if a <code>WEB-INF</code>-based directory
	 *         is requested and the application is being served from a .war file, for instance).
	...
	/**
	 * Determines the log directory. The log directory is determined in this order:
	 * <ol>
	...
	/**
	 * Determines the temp directory. The log directory is determined in this order:
	 * <ol>
	 ...
	 * </ol>
	 * @param context The servlet context from which to retrieve init parameters.
	 * @return A file representing the preferred temp directory, or <code>null</code> if the directory is not available (if a <code>WEB-INF</code>-based directory
	 *         is requested and the application is being served from a .war file, for instance).
	 * @see #getDataDirectory(ServletContext, String, String)
	 */
