...
public class HTTPServlets {
	 ...
	 */
	public static Enumeration<String> getAccept(final HttpServletRequest request) {
	...
	/**
	 * Returns a compatible content type accepted by the given request, based upon the given preferred content type. In particular, if "application/xhtml+xml" is
	 * preferred but not accepted by the requesting user agent, "text/html" is returned. This method is primarily for sending "application/xhtml+xml" content back
	 * to Microsoft Internet Explorer 6, which only supports XHTML when served as "text/html".
	 * @param request The HTTP request object.
	 * @param preferredContentType The content type preferred.
	 * @return The accepted content type, or <code>null</code> if no compatible content type is accepted by the requesting client.
	 */
	/*TODO del this method; IE sends the wildcard in its accept header, claiming that it accepts everything, so we'll just have to do a single check for the one XHTML media type---a general method won't work
	...
	*/
	...
	public static Enumeration<String> getAcceptLanguage(final HttpServletRequest request) {
		return request.getHeaders(ACCEPT_LANGUAGE_HEADER); //return the accept language headers
	}
					...
					if(contentCoding.toString().equalsIgnoreCase(acceptedEncoding) || CharSequences.equals(acceptedEncoding, WILDCARD_CHAR)) { //if this encoding is the correct encoding or the wildcard character
						if(weightedAcceptedEncoding.getQValue() > 0) { //if this is not a zero weight (meaning unaccepted)
							...
							if(!CharSequences.equals(acceptedEncoding, WILDCARD_CHAR)) { //don't exclude this value simply because *;q=0 is present; we exclude non-specified values, anyway
								return false; //this value has been explicitly excluded
							}
						}
					}
				}
	...
	/*TODO fix	
		...
		{
			getLogger().trace("ifModifiedSince: {}", ifModifiedSinceHeader);
		}
		final String ifUnmodifiedSinceHeader=request.getHeader(IF_UNMODIFIED_SINCE_HEADER);
		if(ifUnmodifiedSinceHeader!=null)
		{
			getLogger().trace("ifUnmodifiedSince: {}", ifUnmodifiedSinceHeader);
		}
	*/

	/**
	 * Finds the current locale and returns it as a locale object. The locale is located by the following search order:
	 * <ol>
	 * <li>The request parameters are searched for a string "locale" parameter.</li>
	 * <li>The current session is searched for the <code>Locale</code> object in the attribute, "locale".</li>
	 * <li>The request's cookies are searched for the string value in the cookie, "locale".</li>
	 * <li>The request's default locale is used.</li>
	 * </ol>
	 * @param request The HTTP request object which may contain a session attribute or a cookie with the locale.
	...
	public static Locale getLocale(final HttpServletRequest request) {
		...
		final Object sessionLocale = session.getAttribute(LOCALE_ATTRIBUTE); //try to get the locale from the session TODO use a constant here
		...
		final Cookie localeCookie = getCookie(request.getCookies(), LOCALE_COOKIE); //get the locale cookie, if there is one
	...
	public static void setLocale(final Locale locale, final HttpServletRequest request, final HttpServletResponse response) {
		...
		localeCookie.setMaxAge(999999); //set the cookie's expiration date TODO fix
		response.addCookie(localeCookie); //set the cookie in the response so that it will be available next time
	}

	/**
	 * Determines the raw, unencoded servlet path info. This method is analogous to {@link HttpServletRequest#getPathInfo()} except that this method will return
	 ...
	 * @param request The HTTP request object.
	 * @return The "exception" attribute, or <code>null</code> if there is no exception or the object represented is not an exception.
	...
	/**
	 * Returns the destination indicating the location to which a series of web pages is progressing (e.g. the web page to which the user should be transferred
	...
	/**
	 * Determines the destination indicating the location to which a series of web pages is progressing (e.g. the web page to which the user should be transferred
	 * after login is complete).
	 * <p>
	 * The destination is determined in the following order:
	 * </p>
	 * <ol>
	...
	/**
	 * Returns the destination indicating the location to which a series of web pages is progressing (e.g. the web page to which the user should be transferred
	 * after login is complete). If no destination is available, the redirect goes to the root ("/").
	...
	*/
	...
	//spiders; see https://en.wikipedia.org/wiki/User_agent
	 ...
	 * evolution of the user of the user agent string.
	...
	public static Map<String, Object> getUserAgentProperties(final HttpServletRequest httpRequest) {
		...
		if(userAgent != null) { //if there is a user agent string
			...
			//TODO del getLogger().trace("user agent: {}", userAgent);
			...
			//TODO del getLogger().trace("user agent name: {} with version {} with version numbers {}", userAgentName, userAgentVersion, userAgentVersionNumbers!=null ? Arrays.toString(userAgentVersionNumbers) : null);
	...
	public static void setContentDisposition(final HttpServletResponse response, final ContentDispositionType contentDispositionType, final String filename) {
		final StringBuilder contentDispositionStringBuilder = new StringBuilder(getSerializationName(contentDispositionType)); //start with the disposition type (e.g. "attachment")
		if(filename != null) { //if a filename is provided
			contentDispositionStringBuilder.append("; filename=").append('"').append(filename).append('"');
		}
		response.setHeader(CONTENT_DISPOSITION_HEADER, contentDispositionStringBuilder.toString()); //set the content disposition
	}
	...
	*/
	...
	/**
	 * Sets the session ID of a URI. The session ID is appended to the URI path according to the <cite>Java Servlet Specification 2.4</cite> SRV7.7.1.3 URI
	 * Rewriting. If the URI already contains this session URI designation, the URI is returned unchanged.
	...
	/**
	 * Redirects to the destination, based upon the information given in the request.
	 * <p>
	 * The destination is determined in the following order:
	 * </p>
	 * <ol>
	...
	/**
	 * Redirects to the destination, based upon the information given in the request and in the parameters.
	 * <p>
	 * The destination is determined in the following order:
	 * </p>
	 * <ol>
	...
	/**
	 * Sends content in the response, optionally as an attachment. Caching is turned off.
	 * @param request The HTTP request.
	 ...
	 * @param contentType The MIME type of the content, or <code>null</code> if not known, in which case the <code>application/octet-stream</code> type will be
	 *          used.
	 * @param contentDispositionType The content disposition type.
	 * @param filename The disposition filename, or <code>null</code> if there is no filename to suggest.
	...
	/**
	 * Gets a compressed output stream to the HTTP response, but only if the HTTP request indicates that the user agent supports compression. The
	 * {@value HTTP#CONTENT_ENCODING_HEADER} header is updated if needed.
	 * <p>
	 * Because IE6 with a service pack before 2002-01-08 will ignore the first 2K of compressed data, this method only compresses the data if IE6 indicates that
	 * Service Pack 2 or equivalent is installed by the presence of the {@value #USER_AGENT_MSIE6_SV1} value in the {@value HTTP#ACCEPT_ENCODING_HEADER} header.
	 * </p>
	 * <p>
	 * Technically a {@value HTTP#TRANSFER_ENCODING_HEADER} header should be used for dynamically-compressed data, but currently browsers do not send a
	 * {@value HTTP#TE_HEADER} header to indicate that they can support transfer-encoding data.
	 * </p>
	 * @param request The HTTP request.
	 * @param response The HTTP response.
	...
	public static OutputStream getCompressedOutputStream(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
		...
		if(contentCoding != null) { //if the request accepts gzip or compress
			...
			if(!USER_AGENT_NAME_MSIE.equals(userAgentProperties.get(USER_AGENT_NAME_PROPERTY)) //if this isn't MSIE
				...
				//TODO del getLogger().traceStack("using encoding: {}", contentCoding);
