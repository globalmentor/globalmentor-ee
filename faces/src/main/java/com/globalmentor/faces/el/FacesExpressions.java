...
public class FacesExpressions {
	...
	/**
	 * Determines if the expression conforms to the syntax requirements of a value binding or method binding expression.
	...
	public static boolean isReferenceExpression(final String string) {
		return string.length() >= 3 //if there is enough room for the reference characters
				&& string.charAt(0) == FacesExpressions.REFERENCE_CHAR && string.charAt(1) == FacesExpressions.REFERENCE_EXPRESSION_BEGIN_CHAR
				&& string.charAt(string.length() - 1) == FacesExpressions.REFERENCE_EXPRESSION_END_CHAR; //...}
	}

	/**
	 * Determines if the expression conforms to the syntax requirements of an extended method binding expression.
	 * @param string The string value of the expression.
	 ...
	 * @param expression The expression to convert to a value binding reference.
	 * @return A value binding expression string in the format "#{<var>expression</var>}".
	 */
	public static String createValueBindingExpressionString(final String expression) {
		return new StringBuilder().append(FacesExpressions.REFERENCE_CHAR).append(FacesExpressions.REFERENCE_EXPRESSION_BEGIN_CHAR).append(expression)
				.append(FacesExpressions.REFERENCE_EXPRESSION_END_CHAR).toString();
	}
